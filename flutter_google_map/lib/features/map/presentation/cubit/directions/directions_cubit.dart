...
import '../../../domain/use_case/directions/directions_use_case.dart';
...
class DirectionsCubit extends Cubit<DirectionsState> {
  ...
  DirectionsCubit({required DirectionsUseCase directionsUseCase})
    : _directionsUseCase = directionsUseCase, super(DirectionsInitial());

  Future<void> getDirections(LatLng origin, LatLng destination) async {
    emit(DirectionsLoading());

    try {
      final directions = await _directionsUseCase(
        origin,
        destination,
      );
